 ...
 */
  it('should greet the named user', async () => {
    await AngularHomepage.element(by.model('yourName')).sendKeys('Julie');
    const greeting = AngularHomepage.element(by.binding('yourName'));
...
describe('angularjs homepage', () => {
  ...
  it('should list todos', async () => {
    const todoList = AngularHomepage.element.all(by.repeater('todo in todoList.todos'));
  ...
  it('show to be done items'async () => {  
		//gilt todoList hier auch?
		const tbdItems = AngularHomepage.AngularHomepage.element.all(todoList.by.model("todo.done"));
  });
